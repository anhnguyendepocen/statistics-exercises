...
From the table above we notice that `F-test` results show `Pr(>F) << 0.5`, which suggests that `nihills2.lm`, that is, the most complex model including an interaction term, appears as significantly better than Model 1 for `nihills` data and that we should reject the hypothesis of the coefficient for the interaction term being null.
...
For the model `nihills1.lm` we see that both `Annalong Horseshoe`, `Flagstaff to Carling` and `Seven Sevens` seem problematic, and that residuals are not evenly distributed. We may notice from the Scale-Location plot of `nihills2.lm` that `Slieve Donard` and `Meelbeg Meelmore` also have high residuals. `Seven Sevens` is clearly an outlier, a claim confirmed by the Residuals vs Leverage plot of both models, where the observation is shown to have a large Cook's distance. We consider these observations problematic for our modeling task.
...
After removing the problematic points, the ANOVA test on the new fitted models shows that there is no significant improvement in the more complex model over the simple one.
...
We can observe that for the ridge regression case both `bodywt` and `lsize` coefficients are penalized in favor of the intercept.
...
boot.ci(results1, type="basic")
boot.ci(results2, type="basic")
```

We can compare confidence intervals produced in this way with the one generated by `predict.lm` above. It is interesting to note that the bootstrap-based interval for the linear model is quite similar to the one generated by the function, while the one estimated on the ridge regression using bootstrap has more or less the same amplitude but is shifted below by a factor of roughly 0.5.
...
We can observe that 12, 15 and 17 are influential observation, but there are no real outliers that cross the Cook's distance contours in the Residuals vs Leverage plot.
...
We can see that by plotting residuals against a categorical variable data points can be found only at discrete values of `x7`. It is interesting to note that classes are almost perfectly separated.
...
table(actual=test$clinically.important.brain.injury, predicted=preds)
print(paste("Accuracy of the logistic model:", 1 - mean(preds != test$clinically.important.brain.injury)))
```

We see that using a logistic regression model with a risk threshold of 0.025 and performing a train-test split of 80-20 we achieve an accuracy of 91.34% on the test set.

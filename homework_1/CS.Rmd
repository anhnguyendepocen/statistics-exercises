...
1. The Cumulative Probability Function, P(x), of a random variable x expresses the probability that x does not exceed the value x, as a function of x. $F(x)=P(X \leq x)$ $\forall x$.
...
$$ = E (X)^{2} + 2E(XY) + E(Y)^{2} - [E(Y)]^{2} + 2[E(XY) - E(X)E(Y)]$$
...
$Var(X) = \sigma^{2}_{x}$,
$Var(Y) = \sigma^{2}_{y}$,
$Cov(X,Y) = \sigma^{2}_{xy}$

we have:

$$Var(X+Y)= \sigma^{2}_{x} + \sigma^{2}_{y} + 2\sigma^{2}_{xy}$$
